...
module.exports = class HMrequest {
    static async nekosGet(name) {
        ...
        let res = await fetch(`https://nekos.life/api/v2/img/${name}`).then(res => res.json()).catch(err => `Something wrong with Nekos API: ${console.log(err)}\nContact the support Discord server for help: https://discord.gg/Cak7GBJ`);
        if (blacklist.includes(res.url)) await this.nekosGet(name)
        else return res;
    }
    
    static async missGet(name) {
        let res = await fetch(`https://miss.perssbest.repl.co/api/v2/${name}`).then(res => res.json()).catch(err => `Something wrong with Miss API: ${console.log(err)}\nContact the support Discord server for help: https://discord.gg/Cak7GBJ`)
        return { url: res.image }
    }
    
    static async nekoloveGet(name) {
        return await fetch(`https://neko-love.xyz/api/v1/${name}`).then(res => res.json()).catch(err => `Something wrong with neko-love API: ${console.log(err)}\nContact the support Discord server for help: https://discord.gg/Cak7GBJ`)
    }
    ...
    static async freakerGet(name) {
        ...
        let res = await fetch(`https://api.computerfreaker.cf/v1/${name}`, { method: 'GET', headers: headers }).then(res => res.json()).catch(err => `Something wrong with Freaker API: ${console.log(err)}\nContact the support Discord server for help: https://discord.gg/Cak7GBJ`)
